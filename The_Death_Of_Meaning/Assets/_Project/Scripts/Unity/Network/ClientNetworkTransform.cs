...
namespace TDOM.Unity
{
    ...
    public class ClientNetworkTransform : NetworkBehaviour
    {
        ...
        internal struct FlagStates
        {
            ...
            internal void SetStateFromBitset(uint bitset)
            {
                ...
                SwitchTransformSpaceWhenParented =
                    (bitset & k_SwitchTransformSpaceWhenParented) != 0;
                    ...
                    if (isWriting)
                    {
                        if (FlagStates.UseUnreliableDeltas)
                        {
                            ...
                            if (
                                ...
                                || (
                                    FlagStates.UseHalfFloatPrecision
                                    && NetworkDeltaPosition.CollapsedDeltaIntoBase
                                )
                            )
                            {
                        ...
                        BytePacker.WriteValueBitPacked(
                            m_Writer,
                            FlagStates.GetBitsetRepresentation()
                        );
                    ...
                    if (UseHalfFloatPrecision)
                    {
                        ...
                        if (IsTeleportingNextFrame)
                        {
                            ...
                            if (IsSynchronizing)
                            {
                                ...
                                {
                                    serializer.SerializeNetworkSerializable(
                                        ref NetworkDeltaPosition
                                    );
                                }
                            }
                        }
                    ...
                    if (QuaternionSync)
                    {
                        ...
                        else
                        {
                            ...
                            if (QuaternionCompression)
                            {
                                if (isWriting)
                                {
                                    QuaternionCompressed = QuaternionCompressor.CompressQuaternion(
                                        ref Rotation
                                    );
                                }
                                ...
                                if (!isWriting)
                                {
                                    QuaternionCompressor.DecompressQuaternion(
                                        ref Rotation,
        ...
        public delegate (
            ...
            Quaternion rotOut,
            Vector3 scale
        ) OnClientRequestChangeDelegate(Vector3 pos, Quaternion rot, Vector3 scale);
        ...
        /// </remarks>
        [Tooltip(
            "When enabled the position interpolator will Slerp towards its current target position."
        )]
        ...
        /// </summary>
        private NetworkDeltaPosition m_HalfPositionState = new NetworkDeltaPosition(
            Vector3.zero,
            0
        );
        ...
        protected virtual void OnAuthorityPushTransformState(
            ref NetworkTransformState networkTransformState
        ) { }
        ...
        private void TryCommitTransform(bool synchronize = false, bool settingState = false)
        {
            ...
            {
                NetworkLog.LogError(
                    $"[{name}] is trying to commit the transform without authority!"
                );
                return;
            }
#if COM_UNITY_MODULES_PHYSICS || COM_UNITY_MODULES_PHYSICS2D
            ...
            )
            {
                ...
                if (
                    ...
                    && m_NetworkRigidbodyInternal.NetworkRigidbodyConnections.Count > 0
                )
                {
                    foreach (
                        var childRigidbody in m_NetworkRigidbodyInternal.NetworkRigidbodyConnections
                    )
                    {
                ...
                if (TickSyncChildren)
                {
                    ...
                    foreach (var childNetworkTransform in NetworkObject.NetworkTransforms)
                    {
                        ...
                        if (
                            childNetworkTransform == this
                            || childNetworkTransform.AuthorityMode != AuthorityMode
                        )
                        {
                            continue;
                        }
        ...
        )
        {
            ...
            var isTeleportingAndNotSynchronizing =
                flagStates.IsTeleportingNextFrame && !isSynchronization;
            var isDirty = false;
            var isPositionDirty = isTeleportingAndNotSynchronizing
                ? flagStates.HasPositionChange
                : false;
            var isRotationDirty = isTeleportingAndNotSynchronizing
                ? flagStates.HasRotAngleChange
                : false;
...
#if COM_UNITY_MODULES_PHYSICS || COM_UNITY_MODULES_PHYSICS2D
            if (
                (InLocalSpace != flagStates.InLocalSpace || isSynchronization)
                && !m_UseRigidbodyForMotion
            )
#else
...
#endif
            {
                ...
                flagStates.InLocalSpace = SwitchTransformSpaceWhenParented
                    ? transform.parent != null
                    : InLocalSpace;
                if (SwitchTransformSpaceWhenParented)
                {
                ...
                flagStates.IsTeleportingNextFrame |=
                    !SwitchTransformSpaceWhenParented || isSynchronization;
            ...
            else if (SynchronizePosition)
            {
                ...
                if (!isPositionDirty)
                {
                    ...
                    {
                        if (
                            Math.Abs(position[i] - m_HalfPositionState.PreviousPosition[i])
                            >= positionThreshold[i]
                        )
                        {
                            isPositionDirty =
                    ...
                    else // If synchronizing is set, then use the current full position value on the server side
                    {
                        if (ShouldSynchronizeHalfFloat(targetClientId))
                        {
                            ...
                            if (m_HalfPositionState.NetworkTick > 0)
                            {
                                ...
                                {
                                    networkState.DeltaPosition =
                                        m_HalfPositionState.HalfDeltaConvertedBack;
                                }
                                else
                                {
            ...
            if (!UseQuaternionSynchronization)
            {
                if (
                    SyncRotAngleX
                    && (
                        Mathf.Abs(Mathf.DeltaAngle(networkState.RotAngleX, rotAngles.x))
                            >= rotationThreshold.x
                        || flagStates.IsTeleportingNextFrame
                        || isAxisSync
                        || forceState
                ...
                if (
                    SyncRotAngleY
                    && (
                        Mathf.Abs(Mathf.DeltaAngle(networkState.RotAngleY, rotAngles.y))
                            >= rotationThreshold.y
                        || flagStates.IsTeleportingNextFrame
                        || isAxisSync
                        || forceState
                ...
                if (
                    SyncRotAngleZ
                    && (
                        Mathf.Abs(Mathf.DeltaAngle(networkState.RotAngleZ, rotAngles.z))
                            >= rotationThreshold.z
                        || flagStates.IsTeleportingNextFrame
                        || isAxisSync
                        || forceState
            ...
            else if (SynchronizeRotation)
            {
                ...
                if (!isRotationDirty)
                {
                    ...
                    {
                        if (
                            Mathf.Abs(Mathf.DeltaAngle(previousRotation[i], rotAngles[i]))
                            >= rotationThreshold[i]
                        )
                        {
                            isRotationDirty = true;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void UpdatePositionInterpolator(
            Vector3 position,
            double time,
            bool resetInterpolator = false
        )
        {
            if (!CanCommitToTransform)
            {
                ...
                {
                    m_PositionInterpolator.AutoConvertTransformSpace =
                        SwitchTransformSpaceWhenParented;
            ...
            if (SynchronizePosition)
            {
                ...
                if (flagStates.HasPositionChange || Interpolate)
                {
                    ...
                    else
                    {
                        ...
                        var position = InLocalSpace
                            ? CachedTransform.localPosition
                            : CachedTransform.position;
                        m_InternalCurrentPosition.x = SyncPositionX
                            ? adjustedPosition.x
                            : position.x;
                        m_InternalCurrentPosition.y = SyncPositionY
                            ? adjustedPosition.y
                            : position.y;
                        m_InternalCurrentPosition.z = SyncPositionZ
                            ? adjustedPosition.z
                            : position.z;
                    }
                }
        ...
        private void ApplyTeleportingState(NetworkTransformState newState)
        {
            ...
            if (flagStates.HasPositionChange)
            {
                ...
                else
                {
                    ...
                    if (isSynchronization)
                    {
                        ...
                        {
                            m_HalfPositionState.HalfVector3.Axis = newState
            ...
            if (flagStates.HasRotAngleChange)
            {
                ...
                if (Interpolate)
                {
                    m_RotationInterpolator.AutoConvertTransformSpace =
                        SwitchTransformSpaceWhenParented;
        ...
        private void OnNetworkStateChanged(
            NetworkTransformState oldState,
            NetworkTransformState newState
        )
        {
        ...
        private void AxisChangedDeltaPositionCheck()
        {
            if (UseHalfFloatPrecision && SynchronizePosition)
            {
                ...
                if (
                    SyncPositionX != synAxis.x
                    || SyncPositionY != synAxis.y
                    || SyncPositionZ != synAxis.z
                )
                {
        ...
        protected internal override void InternalOnNetworkPostSpawn()
        {
            ...
            if (
                ...
                && m_CachedNetworkManager.IsConnectedClient
                && SynchronizeState.IsSynchronizing
            )
            {
                NonAuthorityFinalizeSynchronization();
        ...
        private void CleanUpOnDestroyOrDespawn()
        {
            ...
            {
                m_CachedNetworkManager?.NetworkTransformRegistration(
                    m_CachedNetworkObject,
                    forUpdate,
                    false
                );
            }
        ...
        protected virtual void OnInitialize(
            ref NetworkVariable<NetworkTransformState> replicatedState
        ) { }
        ...
        private void InternalInitialization(bool isOwnershipChange = false)
        {
            ...
            if (SwitchTransformSpaceWhenParented)
            {
                if (CanCommitToTransform)
                {
                    ...
                    InLocalSpace =
                        m_CachedNetworkObject
                            .transform.GetComponentsInParent<NetworkObject>()
                            .Length > 1;
                }
        ...
        protected override void OnOwnershipChanged(ulong previous, ulong current)
        {
            ...
            if (
                current == m_CachedNetworkManager.LocalClientId
                || previous == m_CachedNetworkManager.LocalClientId
            )
            {
        ...
        internal override void InternalOnNetworkObjectParentChanged(
        ...
        )
        {
            ...
            if (SynchronizePosition)
            {
                ...
                {
                    m_InternalCurrentPosition = m_LastStateTargetPosition =
                        GetSpaceRelativePosition();
        ...
        )
        {
            ...
            m_LocalAuthoritativeNetworkState.ExplicitSet =
                (stateWasDirty && explicitState) || isDirty;
        ...
        public Dictionary<int, List<NTPositionStats>> PositionStats =
            new Dictionary<int, List<NTPositionStats>>();
        ...
        private void UpdateInterpolation()
        {
            ...
            )
            {
                ...
                var ticksAgo =
                    (!IsServerAuthoritative() && !IsServer ? 2 : 1) + InterpolationBufferTickOffset;
            ...
            if (GatherStats)
            {
                ...
                {
                    posStats.Buffer.Add(
                        new BufferEntry() { TimeSent = entry.TimeSent, Position = entry.Item }
                    );
                }
        ...
        private void UpdateTransformState()
        {
            ...
            {
                Debug.LogError(
                    $"Server authoritative {nameof(NetworkTransform)} can only be updated by the server!"
                );
            }
            else if (!isServerAuthoritative && !IsServer && !IsOwner)
            {
                Debug.LogError(
                    $"Owner authoritative {nameof(NetworkTransform)} can only be updated by the owner!"
                );
            }
            ...
            if (IsServer)
            {
                var clientCount = m_CachedNetworkManager
                    .ConnectionManager
                    .ConnectedClientsList
                    .Count;
                for (int i = 0; i < clientCount; i++)
                {
                    var clientId = m_CachedNetworkManager
                        .ConnectionManager
                        .ConnectedClientsList[i]
                        .ClientId;
        ...
        private static Dictionary<
            NetworkManager,
            NetworkTransformTickRegistration
        > s_NetworkTickRegistration =
            new Dictionary<NetworkManager, NetworkTransformTickRegistration>();
        ...
        internal void RegisterForTickSynchronization()
        {
            s_TickSynchPosition++;
            m_NextTickSync =
                m_CachedNetworkManager.ServerTime.Tick + (s_TickSynchPosition % m_CachedTickRate);
        }
        ...
        private static void RegisterNetworkManagerForTickUpdate(NetworkManager networkManager)
        {
            if (!s_NetworkTickRegistration.ContainsKey(networkManager))
            {
                s_NetworkTickRegistration.Add(
                    networkManager,
                    new NetworkTransformTickRegistration(networkManager)
                );
            }
        }
        ...
        private void DeregisterForTickUpdate()
        {
            ...
            if (s_NetworkTickRegistration.ContainsKey(m_CachedNetworkManager))
            {
                ...
                if (
                    !m_CachedNetworkManager.DistributedAuthorityMode
                    && s_NetworkTickRegistration[m_CachedNetworkManager].NetworkTransforms.Count
                        == 0
                )
                {
