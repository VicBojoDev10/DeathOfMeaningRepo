...
namespace TDOM.Unity
{
    ...
    private void AplicarColorTemporal(CharacterIds id)
    {
        ...
        if (id == CharacterIds.Zendre) _zendreImg.color = colorElegidoPorMi;
        if (id == CharacterIds.Ayla) _aylaImg.color = colorElegidoPorMi;}
    ...
    private void MarcarListo()
    {
        ...
        if (readyButtonText != null) readyButtonText.text = "¡Listo!";
        if (statusText != null) statusText.text = "Esperando al otro jugador...";
    }
    ...
    private void ActualizarUI(NetworkListEvent<PlayerSelectionState> _)
    {
        ...
        foreach (var j in GameSessionManager.Instance.Jugadores)
        {
            ...
            if (j.Character == CharacterIds.Zendre) zendreTomadoPorOtro = true;
            if (j.Character == CharacterIds.Ayla) aylaTomadoPorOtro = true;
        }

        PintarBoton(zendreButton, _zendreImg, CharacterIds.Zendre, miPersonaje, zendreTomadoPorOtro, yaListo);
        PintarBoton(aylaButton, _aylaImg, CharacterIds.Ayla, miPersonaje, aylaTomadoPorOtro, yaListo);
        ...
        if (statusText != null && !yaListo)
            statusText.text = miPersonaje == CharacterIds.None
                ? "Elige tu personaje"
                : "Presiona Listo para confirmar";
    }

    private void PintarBoton(Button btn, Image img, CharacterIds idDeEsteBoton,
        CharacterIds miPersonaje, bool tomadoPorOtro, bool yaListo)
    {
