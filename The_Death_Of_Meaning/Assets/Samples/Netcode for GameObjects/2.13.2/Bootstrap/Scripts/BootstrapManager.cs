...
namespace Unity.Netcode.Samples
{
    ...
    public class BootstrapManager : MonoBehaviour
    {
        private void OnGUI()
        {
            ...
            {
                GUILayout.Label(
                    $"Mode: {(networkManager.IsHost ? "Host" : networkManager.IsServer ? "Server" : "Client")}"
                );
                ...
                if (networkManager.IsClient)
                {
                    if (GUILayout.Button("Random Teleport"))
                    {
                        if (networkManager.LocalClient != null)
                        {
                            ...
                            if (
                                networkManager.LocalClient.PlayerObject.TryGetComponent(
                                    out BootstrapPlayer bootstrapPlayer
                                )
                            )
                            {
